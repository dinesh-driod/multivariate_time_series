...
df = pd.read_csv('opsd_germany_daily-data.csv', index_col='Date', parse_dates=True)
Year = pd.date_range(start='2006-01-01', end='2017-12-31', freq='D')
...
print('Strength of trend for opsd germany dataset is', F)

FS = np.maximum(0, 1-np.var(np.array(R))/np.var(np.array(S+R)))
print('Strength of seasonality for opsd germany dataset is', FS)
